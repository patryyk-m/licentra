...
export function AppCard({ app, onChanged, userRole, currentUser }) {
  const suspendedLabel =
    app.status === 'suspended' && app.suspensionReason === 'plan_quota'
      ? 'suspended (quota)'
      : app.status === 'suspended' && app.suspensionReason === 'admin'
      ? 'suspended (admin)'
      : app.status;
  ...
  const canDeleteApp = isAdmin || isOwner;
  ...
  const deleteApp = async () => {
    const confirmed = window.confirm('delete this app permanently? this cannot be undone.');
    if (!confirmed) return;

    try {
      let res = await fetch(`/api/apps/delete/${app.id}`, { method: 'DELETE', credentials: 'include' });
      let json = await res.json();
      if (isStepUpRequired(res, json)) {
        if (!(await performStepUp())) return;
        res = await fetch(`/api/apps/delete/${app.id}`, { method: 'DELETE', credentials: 'include' });
        json = await res.json();
      }
      if (json.success) {
  ...
  return (
    <Card ref={setNodeRef} style={style}>
      ...
      <CardContent className="flex gap-2">
        ...
        {canDeleteApp && (
          <Button variant="destructive" className="bg-red-600 hover:bg-red-700" onClick={deleteApp}>
            Delete
          </Button>
        )}
      </CardContent>
